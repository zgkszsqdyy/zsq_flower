...
public class ESDao {
	 ...
	 * @return
	 */
	public static eb_shop getEs_ep_id(int es_id){
		Connection conn=Basedao.getconn();
		PreparedStatement ps=null;
		ResultSet rs=null;
		try {
			ps=conn.prepareStatement("select * from EASYBUY_SHOP where es_id=? ");
			ps.setInt(1,es_id);
			rs=ps.executeQuery();
			if(rs.next()){
				eb_shop es=new eb_shop(
	 ...
	 * 
	 */
	public static int insertDD(int oid,String id,String name,String address,int price){
		String sql="insert into EASYBUY_ORDER values(?,?,?,?,now(),?,1,1)";
		Object [] params={oid,id,name,address,price};
		return Basedao.exectuIUD(sql, params);
	}
	/*
	 * 得到序列
